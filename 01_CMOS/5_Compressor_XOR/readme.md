# 4:2 Compressor (XOR–MUX Based) — CMOS (45nm)

CMOS implementation of a 4:2 compressor, designed and simulated in **Cadence Virtuoso** at the **45nm** process node. Instead of the conventional two-full-adder cascade, this compressor is built using **XOR gates for the Sum path** and **2:1 multiplexers for the Carry paths**, which reduces transistor count and critical-path delay — making it well suited for the partial-product reduction stage of the multiplier.
...
This XOR + MUX structure is a well-known low-power/low-transistor-count technique for 4:2 compressors used in high-speed multiplier partial-product reduction trees.

## Files in this Module

| File | Description |
|---|---|
| `Schematic.png` | Transistor-level CMOS schematic: XOR chain (Sum path) + two 2:1 muxes (Carry/Cout paths) |
...
This compressor is part of the `Compressor_XOR` module under `01_CMOS/`, and is the core reduction cell used in the 4×4/8×8 multiplier with MTCMOS. It mirrors the FinFET compressor in the `02_FinFET/` tree, enabling a direct technology comparison.
